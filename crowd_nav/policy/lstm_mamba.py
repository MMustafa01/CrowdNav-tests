...
''''
So from my understanding the LSTM_MambaRL class wont be the class where I define the model, 
...
class LSTM_MambaRL(MultiHumanRL):
    def __init__(self):
        super().__init__()
        self.name = 'Lstm-MambaRL'
    ...
    def configure(self,config):
        ...
        mlp_dims = [int(x) for x in config.get('LSTM_MambaRL', 'mlp_dims').split(', ')]
        d_state =config.getint('LSTM_MambaRL','d_state')
        d_conv =config.getint('LSTM_MambaRL','d_conv') 
        global_state_dim = config.getint('LSTM_MambaRL', 'global_state_dim')
        self.multiagent_training = config.getboolean('LSTM_MambaRL', 'multiagent_training')
