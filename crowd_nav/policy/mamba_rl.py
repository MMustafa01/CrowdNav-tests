...
class ValueNetork(nn.Module):
    ...
    def forward(self,state):
        ...
        h0 = torch.zeros(1, size[0], self.lstm_hidden_dim,device='cuda')
        c0 = torch.zeros(1, size[0], self.lstm_hidden_dim,device='cuda')
        ...
        value = self.mambaLayer(joint_state)
        # print(value.shape)
...
class MambaRL(MultiHumanRL):
    ...
    def configure(self,config):
        ...
        d_state =config.getint('MambaRL','d_state')
        d_conv =config.getint('MambaRL','d_conv') 
        global_state_dim = config.getint('MambaRL', 'global_state_dim')
