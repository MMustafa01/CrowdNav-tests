...
class CrowdSim(gym.Env):
    ...
    def generate_random_human_position(self, human_num, rule):
        ...
        self.speed_up_list =  np.zeros(human_num)
        if rule =='mixed':
    ...
    def testing_suite(self, human,conflict_radius = None, scenarios = ['goal_change'], human_id = None)->None:
        ...
        for scenario in scenarios:
            if radius_check:
                ...
                elif self.testing_scenario[scenario] == self.testing_scenario['sudden_speed_up']:
    ...
    def sudden_speed_up(self, human_id, epsilon = 0.05):
        random_variable = np.random.rand()
        if epsilon > random_variable:
            self.speed_up_list[human_id] = 1
            self.humans[human_id].v_pref = 2*self.humans[human_id].v_pref 
